...
public class JournalWindow : InterfaceWindow
{
    ...
    public List<QuestData> ActiveQuests { get { return activeQuests; } }
    public List<QuestData> CompletedQuests { get { return completedQuests; } }
    public List<QuestData> FailedQuests { get { return failedQuests; } }
    ...
    public List<JournalData> CharactersList { get { return charactersList; } }
    ...
    public List<JournalData> BeastsList {get { return beastsList; }}
    ...
    public List<JournalData> LocationsList {get { return locationsList; }}
    ...
    /// <summary>
    /// Проинициализировать все списки в журнале
    /// </summary>
    public void InitializeJournalData()
    {
        FormDataButtons(activeQuestsPanel, activeQuests);
        FormDataButtons(completedQuestsPanel, completedQuests);
        FormDataButtons(failedQuestsPanel, failedQuests);
    ...
    /// <summary>
    /// Добавить новую информацию в журнал
    /// </summary>
    void AddData(Transform panel, JournalData jData, List<JournalData> jList)
    {
        GameObject button = GameObject.Instantiate(journalButton) as GameObject;
        RectTransform bTransform = button.GetComponent<RectTransform>();
        button.GetComponentInChildren<Text>().text = jData.dataName;
        button.GetComponent<JournalButton>().jData = jData;
        button.transform.parent = panel;
        Vector3 localPos = bTransform.anchoredPosition;
        bTransform.anchoredPosition3D = new Vector3(0f, -30f * (jList.Count + 0.5f), 0f);
        jList.Add(jData);
        bTransform.localScale = new Vector3(1f, 1f, 1f);
        RectTransform rTransform = panel.GetComponent<RectTransform>();
        if (30f * (jList.Count + 0.5f) > rTransform.sizeDelta.y)
    ...
    /// Добавить новую информацию в журнал
    /// </summary>
    void AddData(Transform panel, QuestData jData, List<QuestData> jList)
    {
        GameObject button = GameObject.Instantiate(journalButton) as GameObject;
        RectTransform bTransform = button.GetComponent<RectTransform>();
        button.GetComponentInChildren<Text>().text = jData.dataName;
    ...
    public void JournalDataAddEventHandler(object sender, JournalRefreshEventArgs e)
    {
        ...
        if (string.Equals(e.JData.dataType,"quest"))
        {
            if (!activeQuests.Contains((QuestData)e.JData))
            {
                ...
                AddData(activeQuestsPanel, (QuestData)e.JData,activeQuests);    
            }
        }
        ...
        else if (string.Equals(e.JData.dataType, "character"))
        {
            if (!charactersList.Contains(e.JData))
            {
                ...
                AddData(charactersPanel, e.JData, charactersList);
            }
        }
        ...
        else if (string.Equals(e.JData.dataType, "beast"))
        {
            ...
            {
                SpFunctions.SendMessage(new MessageSentEventArgs("Новая запись в журнале: " + e.JData.dataName, 2, 1.5f));
                AddData(beastsPanel,e.JData, beastsList);
            }
        }
        ...
        else if (string.Equals(e.JData.dataType, "location"))
        {
            ...
            {
                SpFunctions.SendMessage(new MessageSentEventArgs("Новая запись в журнале: " + e.JData.dataName, 2, 1.5f));
                AddData(locationsPanel, e.JData, locationsList);
            }
        }
